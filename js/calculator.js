  ...
  payoffForm.addEventListener('submit', e => {
    ...
    if (isNaN(balance) || isNaN(apr) || isNaN(payment) || payment <= 0) {
      payoffResults.innerHTML =
        '<p class="error">Please enter valid numbers for all fields.</p>';
      return;
    }
    ...
    if (payment <= firstMonthInterest) {
      payoffResults.innerHTML = `
        <p class="error">
          ...
          to cover the first month’s interest of <strong>${formatCurrency(
            firstMonthInterest
          )}</strong>.<br>
    ...
    payoffResults.innerHTML = `
      <p>
        It will take <strong>${months}</strong> month${months === 1 ? '' : 's'} 
        to pay off your balance.<br>
    ...
    // finally, reveal our hidden chart‐container
    chartCanvas.parentElement.classList.add('active');
  });
});
